...
class UtilRedisBase
{
    ...
    protected function __construct($config, $attr = [])
    {
    ...
    public static function getInstance($config, $attr = [])
    {
        ...
        try {
            ...
            $attr['select'] = isset($attr['select']) ? $attr['select'] : 0;
            $attr['select'] = isset($config['select']) ? $config['select'] : $attr['select'];

            $k = md5(json_encode($config) . $attr['select']);

            if (!isset(static::$_instance[$k]) || !(static::$_instance[$k] instanceof static )) {

                static::$_instance[$k] = new static($config, $attr);
                static::$_instance[$k]->k = $k;
                static::$_instance[$k]->dbId = $attr['select'];

                //如果不是0号库，选择一下数据库。
                if ($attr['select'] != 0) {
                    static::$_instance[$k]->select($attr['select']);
                }
            ...
            return ['code' => $errorException->getCode(), ',msg' => $errorException->getMessage()];
        }
    }
     ...
     * @return bool|int
     */
    public function hUpdate($key, $field, $value)
    {
        ...
        if (is_string($value)) {
            ...
            $old_value = $this->redis->hGet($key, $field);
            $old_value = json_decode($old_value, true);

            if (is_array($old_value)) {
                if (isset($old_value[key($value)])) {
                    unset($old_value[key($value)]);
                }
                $old_value[key($value)] = $value[key($value)];
            }
            if (!is_array($old_value)) {
                $old_value = $value;
            }
            ...
            return $this->redis->hSet($key, $field, json_encode($old_value));
        }
    }
    ...
    public function hMget($key, $field)
    {
        ...
        $data = $this->redis->hMget($key, $field);
        foreach ($data as &$datum) {
            $json_de = json_decode($datum, true);
            $datum = $json_de ? $json_de : $datum;
        }
        unset($datum);
        return $data;
    }
