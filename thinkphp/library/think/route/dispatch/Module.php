...
class Module extends Dispatch
{
    ...
    public function init()
    {
        ...
        $controller_result = isset(explode('.',$result[1])[1])?explode('.',$result[1])[1]:(isset(explode('.',$result[1])[0])?explode('.',$result[1])[0]:'index');

        // 是否自动转换控制器和操作名
        $convert = is_bool($this->convert) ? $this->convert : $this->rule->getConfig('url_convert');
        // 获取控制器名
        $controller = strip_tags($controller_result ?: $this->rule->getConfig('default_controller'));

        $this->controller = $convert ? strtolower($controller) : $controller;
        // dump($this->controller );exit;
        // $this->controller = ''
        // 获取操作名
